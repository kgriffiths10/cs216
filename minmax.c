...
/* Student:              */
	...
	for(int i = 0; i < SIZE; i++){
		if (arr[i] < mymin) // mymin smaller than element i 
			mymin = arr[i]; // set mymin as element i
		if (arr[i] > mymax) // mymax greater than element i
			mymax = arr[i];	// set mymax as element i
	}
